...
from utils import last_speech_detected_seconds, text_transcribed, tools_running, generated_sentences, sleep_02, empty_time_speech_detected, move_bonjour_to_generated, empty_data_live_folder
...
class StateMachine:
    ...
    def __init__(self):
        self.states = {
            ...
            "CONV"      : State(number=1, name="CONV",      start_tools=['T0'], stop_tools=['T6', 'T8'], on_enter=(move_bonjour_to_generated,), on_exit=(empty_time_speech_detected,)),
            "LISTEN"    : State(number=2, name="LISTEN",    start_tools=['T1', 'T6', 'T7', 'T8'], on_exit=(self.edit_prompt,)),
            "START_GEN" : State(number=3, name="START_GEN", start_tools=['T2'], stop_tools=['T6', 'T7', 'T8'], on_exit=(sleep_02,)),
            "GEN"       : State(number=4, name="GEN",       on_exit =(self.write_text_to_say, self.update_sentences_said, empty_time_speech_detected)),
    ...
    def update_next_state(self):
        ...
        for potential_next_state, condition in self.conditions[self.current_state.name].items():
            if condition() and condition != self.cond_else:
                print(f"Condition {condition.__name__} is True")
